...
outputs are Figures 8-10, the new Neuro-Economic plots (11-12), and their CSV traces.
"""
...
def _save_fig10(out: Path, tau_r: float, macro: pd.DataFrame, micro: pd.DataFrame) -> Path:
    ...
    state_handles = [Line2D([0], [0], color=color, linewidth=1.2, label=state.title()) for state, color in colors.items()]
    series_handles = [
        Line2D([0], [0], color="0.3", linewidth=0.6, label="Simulation"),
        Line2D([0], [0], color="0.3", linestyle="--", linewidth=1.0, label="Macro model"),
    ]
    state_legend = ax.legend(handles=state_handles, title="State", loc="upper left", frameon=True, fancybox=False, edgecolor="black")
    ax.add_artist(state_legend)
    ax.legend(handles=series_handles, title="Series", loc="upper right", frameon=True, fancybox=False, edgecolor="black")
    fig.tight_layout(pad=0.8)
...
def plot_fig11_strategy_evolution(cfg: Config, out_dir: str | Path) -> Path:
    """Plot the Roth-Erev learning evolution of the robots over time."""
    _paper_style()
    out = ensure_dir(out_dir)
    model = MicroModel(cfg)
    steps = model.world.steps(model.cfg.duration_s)
    ...
    for step in range(steps):
        ...
        if step % stride == 0 or step == steps - 1:
            # Gather average propensities across the swarm
            props = np.zeros(4)
            for a in model.agents:
                props += np.array(a.propensities)
            props /= len(model.agents)
    ...
    ax.plot(df["time_s"], df["prop_20"], label=r"$\tau_r$ 20s (Aggressive)", color="tab:red", linewidth=1.5)
    ax.plot(df["time_s"], df["prop_60"], label=r"$\tau_r$ 60s", color="tab:orange", linewidth=1.5)
    ax.plot(df["time_s"], df["prop_100"], label=r"$\tau_r$ 100s", color="tab:green", linewidth=1.5)
    ax.plot(df["time_s"], df["prop_160"], label=r"$\tau_r$ 160s (Conservative)", color="tab:blue", linewidth=1.5)
    
    ax.set_xlim(0, 20000)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Average Propensity Score")
    ax.set_title("Roth-Erev Learning: Strategy Evolution Over Time")
    ax.legend(loc="upper left", frameon=True, edgecolor="black")
    
    fig.tight_layout(pad=0.8)
    path = out / "fig11_strategy_evolution.png"
    fig.savefig(path, dpi=160)
    plt.close(fig)
    return path


def plot_fig12_energy_divergence(cfg: Config, out_dir: str | Path) -> Path:
    """Plot the energy divergence between the rational Macro baseline and biased Micro swarm."""
    _paper_style()
    out = ensure_dir(out_dir)
    tau_r = float(cfg.get("behaviour", "default_rest_time_s"))
    stride = _stride_steps(cfg)
    
    # Baseline (perfect math)
    macro = MacroModel(cfg, rest_time_s=tau_r).run(stride=stride).trace
    # Biased (new learning/snoozing agents)
    micro = _micro_average(cfg, tau_r, stride) 
    
    fig, ax = plt.subplots(figsize=(6.1, 4.0))
    
    # Plotting energy converted to 10^5 units
    ax.plot(macro["time_s"], macro["energy"] / 1e5, label="Rational Baseline (Macro Model)", color="black", linestyle="--", linewidth=1.5)
    ax.plot(micro["time_s"], micro["energy"] / 1e5, label="Neuro-Economic Swarm (Micro Model)", color="tab:purple", linewidth=1.8)
    
    ax.set_xlim(0, 20000)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel(r"Net Energy ($10^5$ units)")
    ax.set_title("Impact of Cognitive Biases & Learning on Swarm Efficiency")
    ax.legend(loc="upper left", frameon=True, edgecolor="black")
    
    fig.tight_layout(pad=0.8)
    path = out / "fig12_energy_divergence.png"
    fig.savefig(path, dpi=160)
    plt.close(fig)
    return path
...
def make_all_plots(cfg: Config, out_dir: str | Path) -> list[Path]:
    ...
    # Generate New Neuro-Economic Analysis Plots
    p11 = plot_fig11_strategy_evolution(cfg, out)
    p12 = plot_fig12_energy_divergence(cfg, out)
    
    return [p8, csv, p9, p10, p11, p12, macro_csv]
